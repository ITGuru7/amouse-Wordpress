    ...
    <form class="cart" method="post" enctype='multipart/form-data'>
        ...
        if ( $bundled_items ) {
            ...
            foreach ( $bundled_items as $bundled_item ) {
				...
				$full_product_image = wp_get_attachment_url( $bundled_product->get_image_id() );
				$bundled_product_title = $bundled_product->get_title();
				
                ?>
                <tr>
                    <td class="yith-wcpb-product-bundled-item-image"><a href="<?php echo $full_product_image; ?>" title="<?php echo htmlspecialchars( $bundled_product_title ); ?>" data-mfp-src="<?php echo $full_product_image; ?>"><?php echo $bundled_product->get_image() ?></a></td>
                    <td class="yith-wcpb-product-bundled-item-data">
                        <h3><a href="<?php echo $bundled_product->get_permalink() ?>">
                                <?php echo $quantity . ' x ' . $bundled_product_title ?>
                            </a>
                        </h3>
						...
						$bundled_item_price = $bundled_product->get_price();
						if ( ! $bundled_item_price )
							$bundled_item_price = 0;
						
						$bundled_item_price = number_format( $bundled_item_price, 2 );
						
						echo '<span class="bundled-item-price">' . get_woocommerce_currency() . ' $ ' . $bundled_item_price . '</span>';
                        ?>
